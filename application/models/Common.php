...
class Common extends CI_Model
{
    ...
    public function getUniqueSlug($table = '', $field = '', $value = '', $columns = '*')
    {
        ...
        // $data_info = array();
        // echo "select $columns from $table where $field like '" . $value . "%'";
    ...
    public function dateexplode($explby = '-', $date = '')
    {

        $date_expl = explode($explby, $date);
        $newDateString = $date_expl[2] . "-" . $date_expl[1] . "-" . $date_expl[0];
        return $newDateString;

    }
    public function getDateFormat($date, $format = "m-d-Y")
    {

        $date = new DateTime(trim($date));
        $newDateString = $date->format(trim($format));
        return $newDateString;
...
//
    ...
    public function ajaxpagingnation_frontend_new($start, $num_rows, $maxm, $srch_res = '', $fun_name = '', $other_para = '')
    {
        ...
        if ($curr_page != 1) {
            ...
            //$paging .= '<a href="'.site_url($fun_name.'/'.$pages_page."".$other_para).'" class="activelink">&larr; Previous</a>';
            $paging .= '<li>
                                                    <a  href="' . site_url($fun_name . '/' . $pages_page . "" . $other_para) . '" aria-label="Next">
                                                       &laquo;
        ...
        if ($curr_page >= 7) {
            $pages_page = ($curr_page - 6) * $maxm;
            $paging .= '<li ><a href="' . site_url($fun_name . '/0' . "" . $other_para) . '" >1</a></li>
        <li ><a href="' . site_url($fun_name . '/' . $pages_page . "" . $other_para) . '"  >...</a></li>';
        }
        ...
        for ($i = $initial; $i < $max_cnt_limit; $i++) {
            ...
            if ($start == $nxt_start) {
                $pages_page = $i + 1;
                //$paging .= ' <a href="javascript:void(0)" class="activelink"><strong>'.$pages_page .'</strong></a>';
                $paging .= '<li ><a  class="current-page" href="javascript:void(0)">' . $pages_page . '</a></li>';
            } else {
                $pages_page = $i + 1;
                //$paging .= '<a href="'.site_url($fun_name.'/'.$nxt_start."".$other_para).'" class="inactive">'.$pages_page.'</a>';
                $paging .= '<li  ><a  href="' . site_url($fun_name . '/' . $nxt_start . "" . $other_para) . '">' . $pages_page . '</a></li>';
        ...
        if (($curr_page < $max_pages) && (($curr_page + 5) <= $max_pages)) {
            ...
            //$paging .= '<a href="'.site_url($fun_name.'/'.$next_pages."".$other_para).'" class="activelink">...</a>';
            $paging .= '<li  ><a   href="' . site_url($fun_name . '/' . $next_pages . "" . $other_para) . '">...</a></li>';
            $paging .= '<li  ><a   href="' . site_url($fun_name . '/' . ($max_pages - 1) . "" . $other_para) . '">' . $max_pages . '</a></li>';
        }
        if ($curr_page < $max_pages) {
            $pages_page = $curr_page * $maxm;
            //$paging .= '<a href="'.site_url($fun_name.'/'.$pages_page."".$other_para).'" class="activelink">Next &rarr;</a>';
    ...
    public function convert_day_name($daynum = '')
    {

        $daynum = str_replace("1", "Mon", $daynum);
        $daynum = str_replace("2", "Tue", $daynum);
        $daynum = str_replace("3", "Wed", $daynum);
        $daynum = str_replace("4", "Thu", $daynum);
        $daynum = str_replace("5", "Fri", $daynum);
        $daynum = str_replace("6", "Sat", $daynum);
        $daynum = str_replace("7", "Sun", $daynum);

        return $daynum;
    }
    public function convert_day_name_full($sel_day_name = '')
    {
        $sel_day_name = str_replace("Sunday", "Zondag", $sel_day_name);
        $sel_day_name = str_replace("Monday", "Maandag", $sel_day_name);
        $sel_day_name = str_replace("Tuesday", "Dinsdag", $sel_day_name);
        $sel_day_name = str_replace("Wednesday", "Woensdag", $sel_day_name);
        $sel_day_name = str_replace("Thursday", "Donderdag", $sel_day_name);
        $sel_day_name = str_replace("Friday", "Vrijdag", $sel_day_name);
        $sel_day_name = str_replace("Satday", "Zaterdag", $sel_day_name);
    ...
    public function ajaxpagingnation_admin_new2($start, $num_rows, $maxm, $srch_res = '', $fun_name = '', $other_para = '')
    {
