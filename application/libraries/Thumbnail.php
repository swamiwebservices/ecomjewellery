...
class Thumbnail {
    ...
    **/
	...
	function thumbnail_fun($imgfile='') 	{
...
function ImageStringWrap($image, $font, $x, $y, $text, $color, $maxwidth)
{
    ...
    **/
    function process () {
	$fontface="font/".$this->txt_fontface;
      ...
      //  ini_set('max_execution_time',$this->max_execution_time);
        ...
        //if (false) {
				...
				if(isset($this->txt_watermark[0])){	
                if ($this->txt_watermark[0]!='' ) {
                ...
                if (!$this->bicubic_resample) {
                    imagefilledrectangle($this->img["des"], 0, 0, $X_des, $Y_des, $transparent);
                ...
                if ($this->txt_watermark[0]!='' ) {
                   // imagestring ( $this->img["des"], $this->txt_watermark_font, $this->calc_text_position_H() , $this->calc_text_position_V(), $this->txt_watermark,$txt_color);
					// imagestring ( $this->img["des"], $this->txt_watermark_font, $this->calc_text_position_H() , $this->calc_text_position_V()+25, $this->txt_watermark,$txt_color);
		
					 // Replace path by your own font path 
						 imagettftext( $this->img["des"], $this->txt_watermark_font, 0, $this->calc_text_position_H(), $this->calc_text_position_V(), $txt_color, $fontface,	 $this->txt_watermark[0]); 
						 for($k=1;$k<count($this->txt_watermark);$k++)
						 {
						  imagettftext( $this->img["des"], $this->txt_watermark_font, 0, $this->calc_text_position_H_dyanmic($k), $this->calc_text_position_V()+($this->txt_watermark_font+3)*$k, $txt_color, $fontface,	 $this->txt_watermark[$k]); 
						 }
					 // imagettftext( $this->img["des"], $this->txt_watermark_font, 0, $this->calc_text_position_H(), 40, $white, "font/arial.ttf",	 $this->txt_watermark);
                ...
                @imagecopy ($this->img["des"], $this->img["watermark"], $this->calc_position_H (), $this->calc_position_V (), 0, 0, $this->img["x_watermark"], $this->img["y_watermark"]);
                if ($this->txt_watermark[0]!='' ) {
                    imagestring ( $this->img["des"], $this->txt_watermark_font, $this->calc_text_position_H() , $this->calc_text_position_V(), $this->txt_watermark[0], $txt_color); // $this->txt_watermark_color);
                }
        }
    ...
    **/
    function calc_text_position_H () {
        $W_mark =  imagefontwidth  ($this->txt_watermark_font)*strlen($this->txt_watermark[0]);
        $W = $this->img["x_thumb"];
        switch ($this->txt_watermark_Haling) {
             case 'CENTER':
                 ...
                 break;
             case 'RIGHT':
                 $x = $W-$W_mark-($this->txt_watermark_Hmargin)-strlen($this->txt_watermark[0])-1;
                 break;
             default:
             case 'LEFT':
...
function calc_text_position_H_dyanmic ($pos=0) {
        $W_mark =  imagefontwidth  ($this->txt_watermark_font)*strlen($this->txt_watermark[$pos]);
        $W = $this->img["x_thumb"];
        switch ($this->txt_watermark_Haling) {
             case 'CENTER':
                 $x = $W/2-$W_mark/2;
                 break;
             case 'RIGHT':
                 $x = $W-$W_mark-($this->txt_watermark_Hmargin)-strlen($this->txt_watermark[$pos])-1;
                 break;
             default:
             case 'LEFT':
                $x = 0+($this->txt_watermark_Hmargin);
