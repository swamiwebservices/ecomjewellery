...
class Paystack extends CI_Controller {
		...
		$code = $this->controller;
 				...
 				foreach ($_POST as $key => $value) {
						if (substr($key, 0, strlen($code)) == $code) {
							if (!is_array($value)) {
								$this->db->query("INSERT INTO `setting` SET  `group` = '" . $this->common->getDbValue($code) . "', `key` = '" . $this->common->getDbValue($key) . "', `value` = '" . $this->common->getDbValue($value) . "'");
							} else {
								$this->db->query("INSERT INTO `setting` SET   `group` = '" . $this->common->getDbValue($code) . "', `key` = '" . $this->common->getDbValue($key) . "', `value` = '" . $this->common->getDbValue(json_encode($value, true)) . "', serialized = '1'");
							}
						}
					}
	...
	//	print_r($data['records']);
		$data['sub_heading'] = 'Edit Paystack'; 
