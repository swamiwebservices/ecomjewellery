...
class Security extends CI_Controller
{
    ...
    public function addinv()
    {
        ...
        if (isset($_POST['mode']) && $_POST['mode'] == "submitform") {
			...
			$chk_sql = "SELECT * FROM  inventory_master    where  driver_id = '" . $driver_id[0] . "' AND inv_date='".$inv_date."' AND check_in_secu=0 AND check_out_secu=0 ";
        ...
        $data['vehicle'] = $vehicle = $this->common->getAllRow('master_vehicles', $where_cond);
         ...
         $sql = "select user_id,first_name,middle_name,last_name,qr_code, user_type, status,concat(first_name,' ',last_name) as name from  user_master_front where user_type in ('DRI') and  status!='Delete' AND qr_code!='' order by user_type , name asc   ";
    ...
    public function inventory_delete($uuid = 0)
    {
        $session_user_data = $this->session->userdata('user_data');

        $uuid = filter_var($uuid, FILTER_SANITIZE_STRING);

        $chkUserInfo = $this->common->getSingleInfoBy('inventory_master', 'inv_id', $uuid);
        ...
        redirect('security/inventory', 'refresh');
    }
}
