...
class Dashboard extends CI_Controller
{
 //   public $db;
	...
	//	$data['zone_cnt'] = $zone_cnt = $this->common->numRow('master_zone'," WHERE status in ('Active','Inactive') ORDER BY id");
     ...
     $sql = "SELECT  sum(total) as today_total_cod_order FROM `order_master` where date_added like '".$today_date."%' and payment_method='COD' ";
     $query = $this->db->query($sql);
     $query_result = $query->row_array();
     $data['today_total_cod_order'] =  $query_result['today_total_cod_order'];
     $sql = "SELECT sum(total) as today_total_credit_order FROM `order_master` where date_added like '".$today_date."%' and payment_method='Credit' ";
     $query = $this->db->query($sql);
     $query_result = $query->row_array();
     $data['today_total_credit_order'] =  $query_result['today_total_credit_order'];
 
     $sql = "SELECT  sum(total) as today_total_revenue FROM `order_master` where date_added like '".$today_date."%' ";
     $query = $this->db->query($sql);
     $query_result = $query->row_array();
    ...
    $sql = "SELECT  count('') as today_total_order FROM `order_master` where date_added like '".$today_date."%' ";
    $query = $this->db->query($sql);
    $query_result = $query->row_array();
    $data['today_total_order'] =  $query_result['today_total_order'];

    $sql = "SELECT  count('') as today_total_order_pending FROM `order_master` where order_status_id=1 and  date_added like '".$today_date."%' ";
    $query = $this->db->query($sql);
    $query_result = $query->row_array();
    $data['today_total_order_pending'] =  $query_result['today_total_order_pending'];

    
    $sql = "SELECT  count('') as today_total_order_accepted FROM `order_master` where order_status_id in (2,3) and date_added like '".$today_date."%' ";
    $query = $this->db->query($sql);
    $query_result = $query->row_array();
    $data['today_total_order_accepted'] =  $query_result['today_total_order_accepted'];

    $sql = "SELECT  count('') as today_total_order_completed FROM `order_master` where order_status_id in (4) and date_added like '".$today_date."%' ";
    $query = $this->db->query($sql);
    $query_result = $query->row_array();
    ...
    $data['today_date'] = date("d-m-Y");
        ...
        $sql = "SELECT date_format(date_added,'%d') as day1, date_format(date_added,'%d-%m-%Y') as daysmonth, sum(total) as total_perday FROM `order_master` where date_added like '".$data['date_sales_graph_cond']."%' GROUP by daysmonth order by day1 desc";
        $query = $this->db->query($sql);
       ...
       //tope 5 product 
       $sql = "SELECT product_id ,`name`, SUM(quantity) as topproduct FROM `order_product` op ,order_master om where op.order_id=om.order_id and om.date_added like '".$data['date_sales_pie_cond']."%' GROUP by product_id order by topproduct desc limit 0,5 ";
       $query = $this->db->query($sql);
