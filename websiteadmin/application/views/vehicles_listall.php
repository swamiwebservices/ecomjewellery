    ...
    foreach ($results as $key => $value) {
        ...
        ?>
                                <tr class="  border-left-3  <?php echo ($status == "Active") ? 'border-left-success' : 'border-left-danger' ?>  tr<?php echo $this->common->getDbValue($value['status']); ?>">
                                    ...
                                    <td><?php echo $this->common->getDbValue($value['model']); ?></td>
                                    <td><?php echo $this->common->getDbValue($value['company_made']); ?></td>
                                    <td><?php echo $this->common->getDbValue($value['number_plate']); ?></td>
                                    <td><?php echo $this->common->getDbValue($value['size_capacity']); ?></td>
...
if ($status == "Inactive") {echo '<span class="badge badge-light">Inactive</span>';}
        ?>
                                    ...
                                    <td class="text-center">
                                        <div class="list-icons">
                                            <a href="<?php echo site_url($controller . '/editdata/' . $value['vuid']) ?>" class="list-icons-item text-primary-600" data-popup="tooltip" title="" data-original-title="Edit"><i class="icon-pencil7"></i></a>
                                            <a href="#" class="list-icons-item text-danger-600 bootbox_custom" data-vuid="<?php echo $value['vuid'] ?>" data-popup="tooltip" title="" data-original-title="Delete"><i class="icon-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                <?php }?>
    ...
    var table = $('.dashboarddatatable').DataTable({
        ...
        $('.bootbox_custom').on('click', function() {
            var vuid = $(this).data("vuid") // will return the number 123
            bootbox.confirm({
                title: 'Confirm ',
                ...
                callback: function(result) {
                    if (result) {
                        ...
                        location.href = "<?php echo site_url($controller . '/delete_data/') ?>" + vuid;
