    ...
    foreach ($results as $key => $value) {
        ...
        ?>
                                <tr class="  border-left-3  <?php echo ($status == "Active") ? 'border-left-success' : 'border-left-danger' ?>  tr<?php echo $this->common->getDbValue($value['status']); ?>">
                                    ...
                                    <td><?php echo $this->common->getDbValue($value['mobile_number1']); ?></td>
                                    <td><?php echo $this->common->getDbValue($value['company']); ?></td>
                                    <td><?php echo $this->common->getDbValue($value['imei1']); ?></td>
                                    <td><?php echo $this->common->getDbValue($value['serial_number']); ?></td>
...
if ($status == "Inactive") {echo '<span class="badge badge-light">Inactive</span>';}
        ?>
                                    ...
                                    <td class="text-center">
                                        <div class="list-icons">
                                            <a href="<?php echo site_url($controller . '/editdata/' . $value['mpuid']) ?>" class="list-icons-item text-primary-600" data-popup="tooltip" title="" data-original-title="Edit"><i class="icon-pencil7"></i></a>
                                            <a href="#" class="list-icons-item text-danger-600 bootbox_custom" data-mpuid="<?php echo $value['mpuid'] ?>" data-popup="tooltip" title="" data-original-title="Delete"><i class="icon-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                <?php }?>
    ...
    ?>
        ...
        $('.bootbox_custom').on('click', function() {
            var mpuid = $(this).data("mpuid") // will return the number 123
            bootbox.confirm({
                title: 'Confirm ',
                ...
                callback: function(result) {
                    if (result) {
                        ...
                        location.href = "<?php echo site_url($controller . '/delete_data/') ?>" + mpuid;
