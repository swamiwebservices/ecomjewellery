    ...
    foreach ($results as $key => $value) {
        ...
        ?>
                                <tr class="  border-left-3  <?php echo ($status == "Active") ? 'border-left-success' : 'border-left-danger' ?>  tr<?php echo $this->common->getDbValue($value['status']); ?>">
                                    <td><?php echo $this->common->getDbValue($value['first_name']); ?> <?php echo $this->common->getDbValue($value['middle_name']); ?> <?php echo $this->common->getDbValue($value['last_name']); ?></td>
                                    <td><?php echo $this->common->getDbValue($value['email']); ?></td>
                                   <td><?php echo $this->common->getDbValue($value['state_name_en']); ?></br>
                                    <?php echo $this->common->getDbValue($value['district_name_en']); ?></td>
...
<?php 
										if($value['profile_pic']!=''){
											$photo = back_path.'uploads/profile_pics/'.stripslashes($value['profile_pic']);
										} else {
                                   ...
                                   <img src="<?php echo $photo?>" width="100" height="100">                                    
                                    ...
                                    <img src="<?php echo base_url()?>../uploads/driver/<?php echo $this->common->getDbValue($value['qr_code']); ?>" height="70">
                                    <?php } else { ?>
                                    <a href="<?php echo site_url($controller . '/driver_generate_code/' . $value['user_id']) ?>"><strong>Generate</strong></a>
                                    <?php } ?>
                                    </td>
        ...
        ?>
                                    ...
                                    <td class="text-center">
                                        <div class="list-icons">
                                            <a href="<?php echo site_url($controller . '/editdata/' . $value['user_id']) ?>" class="list-icons-item text-primary-600" data-popup="tooltip" title="" data-original-title="Edit"><i class="icon-pencil7"></i></a>
                                            <a href="#" class="list-icons-item text-danger-600 bootbox_custom" data-duid="<?php echo $value['user_id'] ?>" data-popup="tooltip" title="" data-original-title="Delete"><i class="icon-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                <?php }?>
            ...
            bootbox.confirm({
                ...
                callback: function(result) {
                    if (result) {
                        ...
                        location.href = "<?php echo site_url($controller . '/delete_data/') ?>" + duid;
