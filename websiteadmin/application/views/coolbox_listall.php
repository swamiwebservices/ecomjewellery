                    ...
                    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
                        ...
                        <div class="header-elements d-none">
                            <div class="breadcrumb justify-content-center">
                                <a href="<?php echo site_url($controller . "/addcoolbox"); ?>" class="breadcrumb-elements-item">
...
if (isset($results) && sizeof($results) > 0) {
    ...
    <?php $i = 0;
    	foreach ($results as $key => $value) {
			...
			$sql = "select * from coolbox_cust_master where id='" . $value['id'] . "' ";
    ...
    ?>
                                <tr class="  border-left-3  <?php echo ($status == "Active") ? 'border-left-success' : 'border-left-danger' ?>  tr<?php echo $this->common->getDbValue($value['status']); ?>">
                                    <td><?php echo $this->common->getDbValue($value['box_name_en']); ?></td>
                                    <td><?php echo $this->common->getDbValue($value['box_size']); ?></td>
                                    <td>
                                    <?php if($status == "Active"){?>
                                    <strong><a href="<?php echo site_url($controller . '/cust_box_assign/' . $value['id']) ?>">Assign to customer</a></strong>
                                    <?php } ?>
                                    </td>
        ...
        ?>
                                    ...
                                    <td class="text-center">
                                        <div class="list-icons">
                                            <a href="<?php echo site_url($controller . '/editcoolbox/' . $value['id']) ?>" class="list-icons-item text-primary-600" data-popup="tooltip" title="" data-original-title="Edit"><i class="icon-pencil7"></i></a>
                                            <?php if($num_rows==0){?>
                                            <a href="<?php echo site_url($controller . '/delete_box/' . $value['id']) ?>" class="list-icons-item text-danger-600" title="Delete" onClick="return confirm('Are you sure want to delete this record?');"><i class="icon-trash"></i></a>
                                            <?php } ?>
                                        </div>
                                    </td>
                                </tr>
